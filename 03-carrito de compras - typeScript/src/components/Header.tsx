...
type HeaderProps = {
    cart: TCart[]
    removeItem: (id: TData["id"]) => void
    increaceQuantity: (id: TData["id"]) => void
    decrementQuantity: (id: TData["id"]) => void
    clearCart: () => void
}

const Header = ({ cart, removeItem, increaceQuantity, decrementQuantity, clearCart } : HeaderProps) => {
                                                ...
                                                {cart.map(item => (
                                                    <tr key={item.id}>
                                                        ...
                                                        <td className="flex align-items-start gap-4">
                                                            ...
                                                            {item.quantity}
                                                            <button type="button" className="btn btn-dark" onClick={()=>increaceQuantity(item.id)}>
                                                                +
                                                            </button>
                                                        </td>
                                                        <td>
