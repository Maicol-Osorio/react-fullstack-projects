...
const Order = ({item, dispatch}: OrderProps) => {
  return (
    <div className="flex w-full p-3 ring-1 shadow-lg rounded-lg">
      ...
      <div className="flex items-center">
        <span className="bg-red-400 px-3 py-1 rounded-full font-bold hover:bg-red-600 hover:text-white duration-200 cursor-pointer" onClick={()=>dispatch({type: "remove-order", payload: {id: item.id}})}>x</span>
      </div>
    </div>
  )
}
